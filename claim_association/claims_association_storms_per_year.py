...
d_after       = 3
d_before      = 3
type_claim    = 'tempete_extended' # tempete_extended #tempete
# sinclim_vdate = "101225"
...
if type_claim == "tempete" :
    ...
    lib_per_sel = ["tempete", "degat_des_eaux", "orage", "autre"]
...
df_info_storm = df_info_storm.loc[df_info_storm.storm_landing_date < datetime.datetime(year=end_year, 
...
sinclim_raw = sinclim_sort_storm.loc[sinclim_sort_storm.dat_sin < datetime.datetime(year=end_year, month=4, day=1, hour=0)+delta_t_after].copy()
...
path_sinclim_storm_wgust = (
    ...
    + "final_per_year/"
    + f"r{r}/"
    + f"{save_name}_d-{d_before}_d+{d_after}_{track_source}_{input_year}-{end_year}_r{r}.csv"
)
combined_dataset_storm = fct_link_storm_claim.import_wgust_footprint(sinclim_storm, r=r)
if "max_fg10" in combined_dataset_storm: 
...
path_sinclim_storm_unique_wgust = (
    ...
    + "final_per_year/"
    + f"r{r}/"
    + f"{save_name}_d-{d_before}_d+{d_after}_unique-wgust_{track_source}_{input_year}-{end_year}_r{r}.csv"
)
...
path_sinclim_storm_wgust_min_claims = (
    ...
    + "final_per_year/"
    + f"r{r}/"
    + f"{save_name}_d-{d_before}_d+{d_after}_unique-wgust_min{nb_min_claims}_{track_source}_{input_year}-{end_year}_r{r}.csv"
)
