...
for cat in cat_type : 
    for q in quantile : 
        ...
        ### COMPUTE SSI
        r=1300
        variable = "max_fg10" # "max_wind_gust"
        if exposure_bool :
            df_info_storm_SSI = storm_eu.SSI_from_footprint(df_info_storm, 'SSI_FRA_wgust_q'+q, 
                                                                      PATH_FOOTPRINTS, variable, r, 
                                                                      wind_th.fg10, 
                                                                      True, FRA_geo_shp, 
                                                                      True, PATH_TRACKS, f"tracks_{cat}_{period}_info_SSI-{exposure_var}-wgust-q{q}_r{r}", 
                                                                      True, exposure, exposure_var)
            df_info_storm_SSI.to_csv(PATH_TRACKS+f"tracks_{cat}_{period}_info_SSI-{exposure_var}-wgust-q{q}_r{r}.csv" , encoding='utf-8', index=False)
        else : 
            df_info_storm_SSI = storm_eu.SSI_from_footprint(df_info_storm, 'SSI_FRA_wgust_q'+q, 
                                                                      PATH_FOOTPRINTS, variable, r, 
                                                                      wind_th.fg10, 
                                                                      True, FRA_geo_shp, 
                                                                      True, PATH_TRACKS, f"tracks_{cat}_{period}_info_SSI-wgust-q{q}_r{r}", 
                                                                      False)
            df_info_storm_SSI.to_csv(PATH_TRACKS+f"tracks_{cat}_{period}_info_SSI-wgust-q{q}_r{r}.csv" , encoding='utf-8', index=False)